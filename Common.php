...
require(RelativePath . "/Classes.php");
require(RelativePath . "/db_adapter.php");
//End Include Files

//Connection Settings @0-6FE70740
$CCConnectionSettings = array (
    "luisnova" => array(
        "Type" => "MySQL",
        "DBLib" => "MySQL",
//        "Host" => "localhost",
//        "Database" => "softnet_luisnova_db",
		"Host" => "localhost",
        "Database" => "luisnova_db",
        "Port" => "3306",
        "User" => "root",
        "Password" => "",
        "Persistent" => false,
...
//Initialize Common Variables @0-922F9DE2
...
header('Expires: ');
define("TemplatePath", RelativePath . "/html/");
...
$CCSLocales->AddLocale("es", Array("es", "ES", array(1, 0, ""), 2, ",", ".", array("enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"), array("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"), array("domingo", "lunes", "martes", "mi�rcoles", "jueves", "viernes", "s�bado"), array("dom", "lun", "mar", "mi�", "jue", "vie", "s�b"), array("D", "L", "M", "M", "J", "V", "S"), array("dd", "/", "mm", "/", "yyyy"), array("dddd", ", ", "dd", " de ", "mmmm", " de ", "yyyy"), array("H", ":", "nn"), array("H", ":", "nn", ":", "ss"), "", "", 1, false, "", "windows-1252", "CP1252"));
...
$DefaultDateFormat = array("GeneralDate");

$MainPage = new clsMainPage();
//End Initialize Common Variables

//luisnova Connection Class @-F03A0FA7
class clsDBluisnova extends DB_Adapter
{
    function clsDBluisnova()
    {
        $this->Initialize();
    }
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["luisnova"]);
        parent::Initialize();
...
function CCSecurityRedirect($GroupsAccess, $URL)
{
    ...
    {
        if(!strlen($URL))
            $Link = ServerURL . "admin/index.php";
        else
            $Link = $URL;
...
/*if($CCConnectionSettings['luisnova']['User'] != 'luisnova_db'){
  exit;
}*/
...
//CCLoginUser @0-B7314484
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBluisnova();
    $SQL = "SELECT id, Nivel, Password FROM usuarios WHERE Login=" . $db->ToSQL($login, ccsText) . " AND Password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
